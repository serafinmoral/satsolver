...
class solveSATBorradoSet:    
    ...
    @staticmethod
    def calculaForzadas(valores,conjunto):
        lista=[]
        for x in conjunto:
            if len(valores.intersection(x.lista))==0:
                lista.append(x)
...
#            print(current)
            ...
            if (varpos[0] and varneg[0]):
                listpos = solveSATBorradoSet.calculaForzadas(valores,self.clausulasborr[var])
                listneg = solveSATBorradoSet.calculaForzadas(valores,self.clausulasborr[-var])
                for lstp in listpos:
                    for lstn in listneg:
                        clau1 = lstp
                        clau2 = lstn
