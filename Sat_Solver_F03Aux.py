    ...
    for i in range(1,clausulas.nVar+1):
        ...
        if (len(clausulas.posAfirmadas[clausulas.varElimina])>0):
            for varA in clausulas.posAfirmadas[clausulas.varElimina]:            
                ...
                lstposNegadas=clausulas.posNegadas[clausulas.varElimina][:]
                for varN in lstposNegadas:
                    ...
                    if len(listaux)>0:
                        if not(estaContenidaBorrado(listaBorrado,listaux)):
                            listaBorrado.append(listaux)
                        else:
                            contenidas=contenidas + 1
                    else:
                        noGen=noGen+1
                band=False
            for l in listaBorrado:
                if not(estaContenida(clausulas,l)):
                    lclau=Clausula()
                    lclau.propos=l
                    clausulas.agregarClausula(lclau)
                    cont=cont+1
                else:
                    contGen=contGen + 1
        else:
                for varN in clausulas.posNegadas[clausulas.varElimina]:
                        borraPosicion(clausulas,clausulas.lstClausulas[varN],clausulas.varElimina)
        print(i,"Proceso de Borrado Variable: ",clausulas.varElimina,", VAfir=",len(clausulas.posAfirmadas[clausulas.varElimina]),", VNeg=",len(clausulas.posNegadas[clausulas.varElimina]),", TotalC=",len(clausulas.posAfirmadas[clausulas.varElimina])*len(clausulas.posNegadas[clausulas.varElimina]),", CNGPElim=",noGen, ", VR=",cont,", ContBorr=",contenidas,", ContGen=",contGen)
