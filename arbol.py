    ...
    def setvalue(self,c,x):
        if self.var == 0:
            if (len(c)==0):
                ...
                hijo0.listavar = self.listavar - {abs(cval)}
                hijo1.listavar = self.listavar - {abs(cval)}
                if (cval>0):
                    hijo1.setvalue(c,x)
                else:
                    hijo0.setvalue(c,x)
...
#            print ("devuelvo vacio",self.var)
            ...
            (v1, cr1) = self.hijos[1].selectconfig(c)
#            print ("devuelvo positivo",cr1,self.var)
            return (v1,cr1.union({self.var}))
        elif -self.var in c:
            (v2, cr2) = self.hijos[0].selectconfig(c)
#            print ("devuelvo negativo" , cr2,self.var)
            return (v2,cr2.union({-self.var}))
        else:
            t = arbol()
            t.var = self.var
            t.value = 0.0
            (v1, cr1) = self.hijos[1].selectconfig(c)
            (v0, cr0) = self.hijos[0].selectconfig(c)
            t.hijos[0] = v0
            t.hijos[1] = v1
            t.listavar = t.hijos[0].listavar.union({t.var})
#            print("devuelvo dos ",cr0,cr1,self.var)
            
            return (t,cr0.union(cr1))
    ...
    def borraconvar(self,cv):
        if (self.var == 0):
            t = arbol()
            t.listavar = self.listavar -cv
            t.asignaval(self.value*2**(len(self.listavar.intersection(cv))))
        elif self.var in cv:
            return self.hijos[0].suma(self.hijos[1])
        else:
            t = arbol()
            t.listavar = self.listavar - cv
            t.var = self.var
            t.hijos[0] = self.hijos[0].borraconvar(cv)
            t.hijos[1] = self.hijos[1].borraconvar(cv)
            
    def total(self):
        if (self.var == 0):
            return self.value*2**(len(self.listavar))
        else:
            return self.hijos[0].total() + self.hijos[1].total()
            
    def poda(self,epsilon):
        globalsum = self.total()
        self.podarec(epsilon,globalsum)
        
    def podarec(self,epsilon,globalsum):
        if (self.var == 0):
            return True
        else:
            v = self.var
            p1 = self.hijos[0].podarec(epsilon,globalsum)
            p2 = self.hijos[1].podarec(epsilon,globalsum)
            if p1 and p2:
                x1 = self.hijos[0].value
                x2 = self.hijos[1].value
                size = 2**len(self.hijos[0].listavar)
                suma = x1 +x2
                if (x1>0) :
