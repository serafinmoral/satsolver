...
class Clausula:
    ...
    def agregar(self, x):
         ...
         if ((j<0) or (abs(self.lista[0])>abs(x))):
             self.lista.insert(0,x)
             return
         if(abs(self.lista[j])<abs(x)):
             self.lista.append(x)
             return
    ...
    def resolution(self,varb,clau2):
        ...
        i = 0
        j = 0
        while (i<len(self.lista)) and (j<len(clau2.lista)):
            var1 = self.lista[i]
            var2 = clau2.lista[j]
            if (var1 == varb):
                i = i+1
                ...
                j = j+1
                clau.lista.append(var2)
                
        while(i<len(self.lista)):
            if abs(self.lista[i])!=varb :
                clau.lista.append(self.lista[i])
            i = i+1
            
        while(j<len(clau2.lista)):
            if abs(clau2.lista[j])!=varb:
                clau.lista.append(clau2.lista[j])
            j = j+1
        return clau
...
#            var = self.nextVar(noborrad,conjuntovars)[0]
#            
#            
    ...
    def computeComplV(self,y,conjuntovars):
        conjunto = self.conjuntoclau.calculaconjunto(y)
        for z in conjuntovars:
            if y in z:
                conjunto.union(z)
        return conjunto
    ...
    def nextVar(self,noborrad):
        y = noborrad[0]
        conjunto = self.computeComplV(y)
        best = len(self.computeComplV(y))
        
        nbest = y
        for y in noborrad[1:]:
            c2 = self.computeComplV(y)
            z = len(c2)
            if(z<best) :
                nbest=y
                best=z
...
#                print(claures.lista)
                ...
                for i in range(current-1):
                    ...
                    for z in l1:
                        ...
                        if var in z.lista:
#                            xpos += 1
                            xneg *= (2**(len(z.lista)-1)-1)/2**(len(z.lista)-1)
                        elif -var in z.lista:
                            xpos *= (2**(len(z.lista)-1)-1)/2**(len(z.lista)-1)
#                            xneg += 1
                    for z in l2:
                        z = z.reduce(valores)
                        if var in z.lista:
...
# info = leeArchivoSet('bart30.shuffled.cnf')
info = leeArchivoSet('bart30.shuffled.cnf')
